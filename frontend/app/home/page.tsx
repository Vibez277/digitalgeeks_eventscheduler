...
export default function Home() {
  ...
  async function signout(){
    try {
      const res = await fetch(apiEndpoint+"/api/auth/signout",{
          method:"GET",
          headers:{
    ...
    <main className="min-h-screen w-full flex items-center gap-5">
      <div className="main flex-grow px-2">
        ...
        <div className="calendar mt-5">
          ...
          <div className={`mt-5 p-3 flex items-center justify-end`}>
            <button onClick={()=>{
              addModal?.setIsActive(true)
            }} className="py-1 px-2 bg-slate-900 text-white font-semibold rounded-md sm:hidden">
              <p>Add Event</p>
            </button>
            <button onClick={async ()=>{
              await signout();
            }} className="py-1 px-2 bg-red-600 text-white font-semibold rounded-md">
              <p>Logout</p>
            </button>
          </div>
        </div>
